...
impl Ui<'_> {
    ...
    pub fn text_button(&mut self, area: Rect, label: &str, style: TextStyle, colours: ButtonStyle, active: bool, enabled: bool) -> bool {
        ...
        let edge = if active { colours.edge_active } else { colours.edge };
        let radius = self.px(colours.radius);
        if radius > 0 {
            self.painter.rounded_rect(area, radius, self.scale.hairline(), fill, edge);
        } else {
            self.painter.fill_rect(area, fill);
            self.painter.stroke_rect(area, self.scale.hairline(), edge);
        }
        let text = if active { colours.text_active } else { colours.text };
    ...
    pub fn underline_tabs(&mut self, selected: &mut usize, area: Rect, labels: &[&str], style: TextStyle, text: u32, text_dim: u32, bar: u32, bar_height: i32) -> bool {
        if labels.is_empty() {
            return false;
        }
        *selected = (*selected).min(labels.len() - 1);
        ...
        for (index, label) in labels.iter().enumerate() {
            ...
            let colour = if active || self.input.hovering(tab) { text } else { text_dim };
            self.label_styled(tab, label, style, colour, Align::Centre);
            if active {
                let w = (tab.w - inset * 2).max(self.px(24));
                let x = tab.x + (tab.w - w) / 2;
                self.painter.fill_rect(Rect::new(x, tab.bottom() - bar_h, w, bar_h), bar);
            }
    ...
    pub fn segmented(&mut self, area: Rect, labels: &[&str], active: usize, style: TextStyle, colours: ButtonStyle, gap: i32) -> Option<usize> {
        if labels.is_empty() {
            return None;
        }
        let gap = self.px(gap);
    ...
    pub fn stepper(&mut self, area: Rect, value: &str, caption: &str, value_style: TextStyle, caption_style: TextStyle, colours: ButtonStyle, text: u32, text_dim: u32) -> i32 {
        let button_w = area.h;
        let minus = Rect::new(area.x, area.y, button_w, area.h);
        let plus = Rect::new(area.right() - button_w, area.y, button_w, area.h);
        let middle = Rect::new(minus.right(), area.y, (plus.x - minus.right()).max(0), area.h);
        let mut delta = 0;
        ...
        self.text.draw_aligned(&mut self.painter, caption, middle, top, caption_style, text_dim, Align::Centre);
        self.text.draw_aligned(&mut self.painter, value, middle, top + cap_h, value_style, text, Align::Centre);
        delta
    }
    ...
    pub fn track(&mut self, area: Rect, fraction: f32, track: u32, fill: u32) {
        ...
        if w > 0 {
            self.painter.fill_rect(Rect::new(area.x, area.y, w, area.h), fill);
        }
    }
    ...
    pub fn dot(&mut self, cx: f32, cy: f32, radius: f32, colour: u32, ring: Option<(f32, u8)>) {
        if let Some((ring_width, ring_alpha)) = ring {
            self.painter.fill_circle_aa(self.kernel, cx, cy, radius + ring_width, colour, ring_alpha);
        }
        self.painter.fill_circle_aa(self.kernel, cx, cy, radius, colour, 255);
    }

    /// A small square-cornered pill: tinted fill, border, optional leading
    /// dot, text.
    pub fn pill(&mut self, area: Rect, label: &str, style: TextStyle, text: u32, fill: u32, edge: u32, dot: bool) {
        self.painter.fill_rect(area, fill);
        self.painter.stroke_rect(area, self.scale.hairline(), edge);
        let pad = self.px(10);
    ...
    pub fn flat_panel(&mut self, area: Rect, fill: u32, edge: u32, shadow: bool) -> Rect {
        if shadow {
            self.painter.shadow_rect(area, self.px(2), self.px(4), 0x1f262a, 24);
        }
        self.painter.fill_rect(area, fill);
    ...
    pub fn stat(&mut self, area: Rect, caption: &str, value: &str, caption_style: TextStyle, value_style: TextStyle, caption_colour: u32, value_colour: u32) {
        let cap_h = self.text.line_height(caption_style);
        let val_h = self.text.line_height(value_style);
        let gap = self.px(3);
        let top = area.y + (area.h - cap_h - val_h - gap) / 2;
        self.text.draw_fitted(&mut self.painter, caption, area, top, caption_style, caption_colour, Align::Left);
        self.text.draw_fitted(&mut self.painter, value, area, top + cap_h + gap, value_style, value_colour, Align::Left);
    }
}
